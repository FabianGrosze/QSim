...
!! Quelle: kombitransi.f95 zurück zu \ref Transportinformationen
      ...
      if(errcode .ne. 0)call qerror('kombitransi writing system call stat failed')
     ...
     &        start, ' bis', ende, 'in regelmäßigen Schritten von  ',dttrans, ' Sekunden'
      if(ende.gt.0)then ! Transportinformationen im Sammelverzeichnis enthalten?
         sammel_start=start
         sammel_ende=ende
         sammel_dt=dttrans
      end if !
      ...
      do ! nächstes Verzeichnis zum Kombinieren
         ...
         if(errcode .ne. 0)call qerror('kombitransi writing system call stat2 failed')
         call system(systemaufruf,sysa)
         if(sysa.ne.0)then
     ...
     &      'cp ',trim(modellverzeichnis),'transinfo/t',i,trim(sammelverzeichnis),'transinfo/'
            if(errcode .ne. 0)call qerror('kombitransi writing system call cp failed')
            call system(systemaufruf,sysa)
            if(sysa.ne.0)then
...
!----+-----+----
      ...
      integer , allocatable , dimension (:) :: transinfo_zuord
         ...
         write(systemaufruf,'(3A)',iostat = errcode)'stat ',trim(modellverzeichnis),' >/dev/null 2>/dev/null'
         if(errcode .ne. 0)call qerror('transinfo_sichten writing system call stat failed')
         call system(systemaufruf,sysa)
         !print*,'sysa',sysa
         if(sysa.ne.0)then
      ...
      if(errcode .ne. 0)call qerror('transinfo_sichten writing system call ls failed')
      !print*,systemaufruf
      ...
      do while (naechste_zeile(ion,ctext))
         nz=nz+1
         if(ctext(1:1).eq.'t')transinfo_anzahl=transinfo_anzahl+1
      end do ! while Zeile
      if(transinfo_anzahl.lt. 1)then
...
!! \n\n
      logical function naechste_zeile(ion,ctext)
         ...
         do while( naechste ) !! nächste nicht Kommentar- und nicht Leer-Zeile auffinden:
            ...
            if(io_error.ne.0) then ! keine Zeile mehr vorhanden.
            ...
            kommentar=.false.
            if (ctext(1:1).eq.'#') kommentar=.true. ! Kommentarzeile, nächste Zeile probieren
