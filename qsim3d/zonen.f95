...
subroutine zonen_parallel()
   ...
   if (meinrang /= 0) then ! alle Prozesse ausser 0
      ...
      if (alloc_status /= 0) call qerror("Error while allocating variable `knoten_zone`.")
      ...
      allocate(point_zone(number_plankt_point), source = 0,  stat = alloc_status )
      if (alloc_status /= 0) call qerror("Error while allocating variable `point_zone`.")
      
      allocate (zone(zonen_anzahl), stat = alloc_status )
      if (alloc_status /= 0) call qerror("Error while allocating variable `zone`.")
   endif
