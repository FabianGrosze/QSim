...
if(meinrang.eq.0)then ! nur auf Prozessor 0 bearbeiten
      ...
      do 7227 iWETT_T = 1,iWETTs_T ! Schleife über alle Wetterstationen
         ...
         if(mWetts_T(iwett_T).ge.mwettmax_T)mwettmax_T=mWetts_T(iwett_T)
         if(mWetts_T(iwett_T).le.0)then
...
!           Tagesmittelwerte
 ...
 2023 format(i2,2x,i2,2x,I4,2x,f5.2,2x  &
            ,f7.2,2x,  f6.2,2x,f6.2,2x,  f6.2,2x,f6.2,2x,f4.1,2x,f4.1)                                         
               ...
               if(wertw_T(iwett_T,3,mwett_T).gt.hcTmx2_T)hcTmx2_T = wertw_T(iwett_T,3,mwett_T)
...
!           Fehlermeldung keine Minimumtemperaturen an einer oder mehrer Wetterstationen
            if(hcTmx2_T.eq.(-1.))then
               ifehl_T = 4
               ifhStr_T = IWETT_T
!              goto 989 -->Fehlermeldung
               write(fehler,*)'Fehler wetter_qsim hcTmx2.eq.(-1.)'
               call qerror(fehler)
            endif
         else
...
!   229      FORMAT ("wetter_readallo0: ",I4.2,"-",I2.2,"-",I2.2," ",I2.2,":",I2.2,":",I2.2,"    ",F7.2,"    ",F7.2)
         ...
         do mWett_T = 2,mWetts_T(iwett_T) !! Zeitfolge prüfen:
             if(zeitpunktw(iwett_T,mwett_T).le.zeitpunktw(iwett_T,mwett_T-1))then
...
!     allokieren der Felder für die Momentan-Werte
      ...
      do i=1,zonen_anzahl
         ...
         do j=1,iWETTs_T
            if( zone(i)%wettstat%wetterstations_nummer.eq.Wetterstationskennung_T(j)) then
...
!! \n\n
      ...
      do i=1,part ! Alle Knoten auf diesem Prozessor 
         ...
         if(iglob.le.number_plankt_point)then ! Knotennummer existiert (letzter Prozess)
            ...
            if(iglob.eq.kontrollknoten)print*,'update_weather: ',tlmed_T(i2),schwi_T(i2),i2,iglob,i,' auf Prozess #',meinrang
...
!! \n\n
      subroutine wettles_wetter()
         ...
         do ipw = 1,7   !! Schleife über alle 7 Wetterwerte
            if( (zeitpunkt.lt.(zeitpunktw(i,1)-43200)) .or. (zeitpunkt.gt.(zeitpunktw(i,mwetts_T(i))+43200)) )then
            ...
            if((.not.found1) .and. (.not.found2)) then
               ywert = w1
               if(ipw.eq.7)then ! wolkentyp darf negativ = ungültig
                  ywert = w1
               else
            ...
            if(ipw.eq.1)glob_T(i) = ywert 
            if(ipw.eq.2)tlmax_T(i) = ywert 
            if(ipw.eq.3)tlmin_T(i) = ywert 
            if(ipw.eq.4)ro_T(i) = ywert 
            if(ipw.eq.5)wge_T(i) = ywert 
            if(ipw.eq.6)cloud_T(i) = ywert 
            if(ipw.eq.7)typw_T(i) = ywert 

         end do ! Schleife über alle 7 Wetterwerte
         if(meinrang.eq.0)print*,'wettles_wetter: Wetterstation,zeitpunkt=',i,zeitpunkt,' Werte='  &
...
!----+-----+----
      ...
      case(6) ! cloud_T(i) = ywert 
         if((wert.ge. 0.0).and.(wert.le. 8.0))wert_gueltig=.true.
