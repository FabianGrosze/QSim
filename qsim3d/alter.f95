...
!     i ist die lokale Knotennummer auf dem jeweiligen Prozessor und läuft von 1 bis part
      ...
      if (iglob.gt. number_plankt_point) return ! überstehende Nummern nicht bearbeiten.
      ...
      if(depth .ge. 0.0 ) then !! knoten nass
...
!! läuft nur auf prozessor 0
      ...
      if(open_error.ne.0)call qerror('alter.txt lässt sich nicht öffnen ???')

      do while( zeile(ion) .and. (wie_altern .eq. 0) )
         if (.not.leerzeile())then
            !i=i+1
            write(b,'(A)')trim(ctext)
            !print*,i," : ",trim(b)
            if((b(1:1).eq."z").or.(b(1:1).eq."Z"))wie_altern=1
            if((b(1:1).eq."r").or.(b(1:1).eq."R"))wie_altern=2
      ...
      case(1) ! Zone
         ...
         if(read_stat.ne.0) call qerror("Lesen der Zonen-zeile:", trim(b)," in Datei alter.txt fehlgeschlagen")
         print*,"Aufenthaltszeiten-Berechnung für zone Nummer:",alter_nummer
         print*," " 
         do i=1,zonen_anzahl ! alle i zonen
            if(alter_nummer.eq.zone(i)%zonen_nummer)altzaehl=i
         end do ! alle i zonen
         alter_nummer=altzaehl
         print*,'selbige Zone hat den Zähler:  ',alter_nummer
         if((alter_nummer.lt.1).or.(alter_nummer.gt.zonen_anzahl)) &
     &      call qerror("alter_lesen(): Zonen-Nummer ungültig")
      case(2) ! Rand
         !print*,"R:",trim(b)
         read(b(2:300), *, iostat = read_stat ) alter_nummer
         if(read_stat.ne.0) call qerror("Lesen der Rand-zeile", trim(b), " in Datei alter.txt fehlgeschlagen")
