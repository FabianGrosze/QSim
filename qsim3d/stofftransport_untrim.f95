...
!----+-----+----
      ...
      real      :: c
         ...
         do n=1,n_elemente ! 
            if ((el_vol(n).le. 0.0).or.(el_vol(n).gt. 1.e+30)) el_vol(n)=0.0
         ...
         do n=1,n_elemente ! 
            if( abs(p(n)) .gt.  1.e+30)p(n)=-999.9
         end do ! alle n elemente
     ...
     &                   ed_vel_x(elementedges(n,k)),ed_vel_y(elementedges(n,k)),elementedges(n,k),n,k
         ...
         do j=1,number_plankt_point ! all j elements
            ...
            if ((el_area(j) .gt. 0.0).and.(el_vol(j).gt. 0.0)) &
      ...
      do n=1,kantenanzahl ! edge fluxes  
         ...
         do n=1,n_elemente ! gathering inflows
            do k=1,cornernumber(n) 
               ...
               if( (left_element(elementedges(n,k)).eq.n).and.(ed_flux(elementedges(n,k)).gt. 0.0) )  &
     &            wicht((n-1)*5+(1+k))= ed_flux(elementedges(n,k))
               !if I am the right_element of my n-th edge and the flux is to the right
               if( (right_element(elementedges(n,k)).eq.n).and.(ed_flux(elementedges(n,k)).lt. 0.0) )  &
     &            wicht((n-1)*5+(1+k))= -1.0 * ed_flux(elementedges(n,k))
         ...
         do n=1,n_elemente ! Gewichtsfaktoren aus Volumenverhältnissen ermitteln
            if(el_vol(n).gt. 0.0)then ! Element wet
               ...
               if(cu(n).gt.cu_max)cu_max=cu(n)
               if(cu(n).lt.cu_min)cu_min=cu(n)
            ...
            sumwicht=0.0
            do j=1,5 !clipping ######
               if(wicht((n-1)*5+j).lt. 0.0) wicht((n-1)*5+j)=0.0
               if(wicht((n-1)*5+j).gt. 1.0) wicht((n-1)*5+j)=1.0
               sumwicht=sumwicht+wicht((n-1)*5+j)
            end do !alle 5
            if(sumwicht.gt.0.0)then
               do j=1,5 !
...
!----+-----+----
      ...
      if(errcode .ne. 0)call qerror('read_mesh_nc writing filename transport.nc failed')
      ...
      if(alloc_status.ne.0) call qerror('allocate (knoten_x failed')
      ...
      if(alloc_status.ne.0) call qerror('allocate (knoten_y failed')
      ...
      if(alloc_status.ne.0) call qerror('allocate (knoten_z failed')
      ...
      if(alloc_status.ne.0) call qerror('allocate (knoten_zone( failed')
      ...
      do n=1,knotenanzahl2D
         if(knoten_x(n).gt.maxx)maxx=knoten_x(n)
         if(knoten_y(n).gt.maxy)maxy=knoten_y(n)
         if(knoten_x(n).lt.minx)minx=knoten_x(n)
         if(knoten_y(n).lt.miny)miny=knoten_y(n)
      end do ! alle Knoten
      ...
      if(alloc_status.ne.0) call qerror('allocate element_xy failed')
      ...
      if(alloc_status.ne.0) call qerror('allocate fa_no( failed')
      allocate (elementnodes(n_elemente,4), stat = alloc_status )
      if(alloc_status.ne.0) call qerror('allocate (elementnodes( failed')
      allocate (cornernumber(n_elemente), stat = alloc_status )
      if(alloc_status.ne.0) call qerror('allocate (cornernumber( failed')
      ...
      do n=1,n_elemente ! alle Elemente
         ...
         do k=1,cornernumber(n)
            ...
            if((elementnodes(n,k).lt. 1).or.(elementnodes(n,k).gt.knotenanzahl2D))then
      ...
      if(alloc_status.ne.0) call qerror('allocate (knot_ele(knotenanzahl2D) failed')
      do n=1,knotenanzahl2D
         knot_ele(n)=0
      end do ! alle Knoten
      ...
      if(alloc_status.ne.0) call qerror('allocate (element_rand failed')
      ...
      if(alloc_status.ne.0) call qerror('allocate (elementedges failed')
      elementedges=0
      ...
      do n=1,n_elemente ! alle Elemente
         do k=1,cornernumber(n)
            ...
            if((elementedges(n,k).lt. 1).or.(elementedges(n,k).gt.kantenanzahl))then
      ...
      if(alloc_status.ne.0) call qerror(' allocate (top_node failed')
      allocate (bottom_node(kantenanzahl), stat = alloc_status )
      if(alloc_status.ne.0) call qerror('allocate (bottom_node failed')
      allocate (ed_fa(2,kantenanzahl), stat = alloc_status )
      if(alloc_status.ne.0) call qerror('allocate (ed_fa(2 failed')
      ...
      do n=1,kantenanzahl ! alle Kanten
         ...
         if((bottom_node(n).lt. 1).or.(bottom_node(n).gt.knotenanzahl2D))call qerror("read_mesh_nc:bottom_node falsch")
         top_node(n)=ed_fa(2,n)+1
         if((top_node(n).lt. 1).or.(top_node(n).gt.knotenanzahl2D))call qerror("read_mesh_nc:top_node falsch")
      end do ! alle n Kanten
...
!     Mesh2_edge_bc:flag_values = 0, 1, 2 ;
      ...
      if(alloc_status.ne.0) call qerror('allocate (nbc( failed')
      call check_err(  nf90_inq_varid(ncid,'Mesh2_edge_bc', didi) )
      call check_err(  nf90_get_var(ncid, didi, nbc ) )
      do n=1,kantenanzahl ! alle Kanten
         if(boundary_number(n).eq. 0)then ! Kante mit zwei Elementen links und rechts
            if(nbc(n).ne. 0)call qerror("Randkantenfehler 00")
         else ! Kante mit nur einem Element
            if(nbc(n).eq. 0)call qerror("Randkantenfehler 11")
      ...
      if(alloc_status.ne.0) call qerror('allocate (intereck(4  failed')
      ...
      do n=1,n_elemente ! alle Elemente
         ...
         do k=1,cornernumber(n)
            ...
            if(boundary_number(elementedges(n,k)).ge. 2) then !! nur Zuflussrand aus Kanten in Elemente übernehmen !!
               ! bei mehreren Randkanten am Element, element_rand auf die größte Kanten-Randnummer setzen
               if(boundary_number(elementedges(n,k)).gt.element_rand(n))then ! winner takes it all
                  element_rand(n)=boundary_number(elementedges(n,k))
               end if ! winner
            end if ! Zufluss-Randkante
         end do ! alle k Kanten im Element
         if(ndumm .gt. 0) then ! Element hatte schon Randnummer
            if(ndumm.ne.element_rand(n))then ! Randunummernkonflikt vermeiden
...
!! selbige wurde von Gerris erzeugt.  
      ...
      if(errcode .ne. 0)call qerror('read_elemente_gerris writing filename ELEMENTE.txt failed')
      ...
      if( (io_error.ne.0).or.( nelli.ne.n_elemente) )then
      ...
      do n=1,n_elemente ! alle Elemente
         ...
         if(n.ne.neln)call qerror('Elementnummer in ELEMENTE.txt falsch')
         dist=(elx-element_x(n))**2 + (ely-element_y(n))**2
         if(dist.gt.25)call qerror('Elementzentren mehr als 5 Meter Entfernung')
...
!     allocate (cell_bound_length(kantenanzahl), stat = alloc_status )
      ...
      if(alloc_status.ne.0) call qerror('allocate (rbc failed')
      do n=1,knotenanzahl2D
         rbc(n) =0.0
      end do ! alle Knoten
      do n=1,kantenanzahl
      ...
      do n=1,n_elemente ! alle Elemente
         do k=1,cornernumber(n)
            ...
            if( element_zone(n).gt.knoten_zone(elementnodes(n,k)) ) knoten_zone(elementnodes(n,k))=element_zone(n)
            !! Randknoten auf Element-nummern setzen:
            if((knoten_rand(elementnodes(n,k)).gt.0).and.(element_rand(n).gt.0))knoten_rand(elementnodes(n,k))=element_rand(n)
...
if(meinrang.eq.0)then ! prozess 0 only
     ...
     &      ,zeitstunde(transinfo_anzahl),' h'
      ...
      if(io_error.ne.0)call qerror('nc_sichten: time_offset-Lesefehler')
      call sekundenzeit(1)
...
!! nv_read.f95
      ...
      write(dateiname,'(2A)',iostat = errcode)trim(modellverzeichnis),'transport.nc'
      if(errcode .ne. 0)call qerror('nvread writing filename transport.nc failed')
      iret = nf_open(dateiname, NF_NOWRITE, ncid)
      call check_err(iret)
...
!-----------------------------------------------------------------------
      ...
      if(nt.gt. 0)then ! there are timesteps
         write(zahl,*)nt
         zahl=adjustl(zahl)
         write(dateiname,'(4A)',iostat = errcode)trim(modellverzeichnis),'netcdf_face_',trim(zahl),'.vtk'
         if(errcode .ne. 0)call qerror('untrim_vtk writing filename netcdf_face failed')
      else ! no timesteps
         RETURN
      end if
      write(systemaufruf,'(2A)',iostat = errcode)'rm -rf ',trim(dateiname)
      if(errcode .ne. 0)call qerror('untrim_vtk writing systemcall rm -rf dateiname failed')
...
!------------------------------------------------------------------------------------------ edges
      ...
      if(nt.gt. 0)then ! there are timesteps
         ...
         if(errcode .ne. 0)call qerror('untrim_vtk writing filename netcdf_edge_ failed')
      else ! no timesteps
         RETURN
      end if
      write(systemaufruf,'(2A)',iostat = errcode)'rm -rf ',trim(dateiname)
      if(errcode .ne. 0)call qerror('untrim_vtk writing systemcall rm -rf dateiname edges failed')
      call system(systemaufruf)
      ion=106
      open ( unit =ion , file = dateiname, status ='unknown', action ='write ', iostat = open_error )
      if(open_error.ne.0) then
...
!-------------------------------------------------------------------------------------------- nodes
      ...
      if(errcode .ne. 0)call qerror('netcdf_mesh_only writing filename netcdf_mesh_node.vtk failed')
      write(systemaufruf,'(2A)',iostat = errcode)'rm -rf ',trim(dateiname)
      if(errcode .ne. 0)call qerror('untrim_vtk writing systemcall rm -rf dateiname nodes failed')
      call system(systemaufruf)
      ion=106
      open ( unit =ion , file = dateiname, status ='unknown', action ='write ', iostat = open_error )
      if(open_error.ne.0) then
...
!-------------------------------------------------------------------------------------------- faces=elements
      ...
      if(errcode .ne. 0)call qerror('netcdf_mesh_only writing filename netcdf_mesh_element.vtk failed')
      write(systemaufruf,'(2A)',iostat = errcode)'rm -rf ',trim(dateiname)
      if(errcode .ne. 0)call qerror('netcdf_mesh_only writing system call rm -rf dateiname failed')
      call system(systemaufruf)
      ion=106
      open ( unit =ion , file = dateiname, status ='unknown', action ='write ', iostat = open_error )
      if(open_error.ne.0) then
...
!Dimensionen: nMesh2_face       11103
      ...
      do n=1,kantenanzahl2D
         if(ed_fa(2,n).le. 0) ed_fa(2,n)=ed_fa(1,n)
         if(ed_fa(1,n).le. 0) ed_fa(1,n)=ed_fa(2,n)
