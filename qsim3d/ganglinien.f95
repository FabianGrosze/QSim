...
!! \n\n
      subroutine ganglinien_parallel()
         ...
         agp=0 !!
         do i=1,anz_gangl
            if((knot_gangl(i).gt.meinrang*part).and.(knot_gangl(i).le.(meinrang+1)*part))then
               agp=agp+1
...
!! \n\n
      subroutine ganglinien_lesen()
         ...
         if(meinrang.gt.0)then !! has to be on process 0
...
!! \n\n
      subroutine ganglinien_zeitschritt(izeit_gang)
         ...
         if((nk.gt.0).and.(nk.le.part))then
...
!    &              ' macht Prozess #',meinrang, ' part=',part,' zeitpunkt=',zeitpunkt
            ...
            do k=1,number_benth_distr
               if(output_benth_distr(k))then ! benth. distr. output
                  n=n+1
                  if(n.gt.n_bn) call qerror('ganglinien_zeitschritt n.gt.n_bn')
            ...
            do k=1,number_trans_val
               if(output_trans_val(k))then ! globale Übergabe Werte
                  n=n+1
                  if(n.gt.n_ue) call qerror('1 ganglinien_zeitschritt (n.gt.n_ue) ')
            ...
            do k=1,number_trans_quant
               if(output_trans_quant(k))then ! exchange con. output
                  n=n+1
                  if(n.gt.n_ue) call qerror('2 ganglinien_zeitschritt (n.gt.n_ue) ')
            ...
            do k=1,number_trans_quant_vert
               if(output_trans_quant_vert(k))then ! exchange con. output
                  n=n+1
                  if(n.gt.n_ue) call qerror('3 ganglinien_zeitschritt (n.gt.n_ue) ')
      ...
      if(hydro_trieb.eq. 1)then ! hydro_trieb=1=casu
         if( nur_alter ) call alter_zeitschritt(izeit+1)
         !   call tagesmittelwert()
         if(n_pl.gt.200)call qerror("n_pl.gt.200 geht nicht wegen fester Feldgröße massen_flux")
...
if(meinrang.eq.0)then ! prozess 0 only
         if(time_style.eq.0)print*,"ganglinien_schliessen: Ausgabeformat Stil: portal-Tideelbe + gerris"
         if(time_style.eq.1)print*,"ganglinien_schliessen: Ausgabeformat Stil wsa_cux"
         if(time_style.eq.2)print*,"ganglinien_schliessen: Ausgabeformat Stil SCHISM"

         !write(dateiname,'(2A)')trim(modellverzeichnis),'ganglinien_knoten.txt'
         write(systemaufruf,'(3A)',iostat = errcode)'rm -rf ',trim(modellverzeichnis),'ganglinien_bisher'
         if(errcode .ne. 0)call qerror('ganglinien_schliessen writing systemcall rm -rf ganglinien_bisher failed')
         call system(systemaufruf,sys_error)
         if(sys_error.ne.0)then
     ...
     &                             ,trim(modellverzeichnis),'ganglinien_bisher >/dev/null 2>/dev/null'
         if(errcode .ne. 0)call qerror('ganglinien_schliessen writing systemcall mv ganglinien failed')
         call system(systemaufruf,sys_error)
         if(sys_error.ne.0)then
         ...
         if(errcode .ne. 0)call qerror('ganglinien_schliessen writing systemcall mkdir ganglinien failed')
         call system(systemaufruf,sys_error)
         if(sys_error.ne.0)then
 ...
 124  continue
        ...
        call MPI_Bcast(beschriftung,40000,MPI_CHARACTER,0,mpi_komm_welt,ierr)
         ...
         if(knot_gangl(i).gt.0)then !Knoten auf diesem Prozess
            ...
            if(errcode .ne. 0)call qerror('ganglinien_schliessen writing filename ganglinien/g*.txt failed')
