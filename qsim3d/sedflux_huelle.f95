...
!! 
!! Das Modul sedflux() dient dazu, die Stofflüsse in das und aus dem Sediment zu simulieren. Nachstehend das Beispiel der gelösten organischen Kohlenstoffverbindungen:\n
!! <center> 
...
!#    subroutine Sed_POM:    w2(mstr,mSta) = 0.74e-5
     ...
     &              ,raua,vmq,Hmq,nbuhn,bvmq,bHmq                  &
     &              ,0,hw2,w2b,ifehl)
      if(ifehl.ne. 0)call qerror('Sediment ifehl')
      ...
      if(zone(point_zone(iglob))%sediflux%kornd.gt. 0.0)  hdkorn(1,1:2) = zone(point_zone(iglob))%sediflux%kornd ! einlesen optional, in 3D neu
