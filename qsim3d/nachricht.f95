...
!        Laufzeitermittlung starten
         ...
         if(errcode .ne. 0)call qerror('fortschritt systemaufruf date start')
...
!        source-code Verweis übernehmen:
      ...
      case (-1) ! end, finalizing
         ...
         if(errcode .ne. 0)call qerror('fortschritt systemaufruf date ende')
         call system(systemaufruf,system_error)
         if(system_error.ne.0)then
     ...
     &                     ,trim(modellverzeichnis),'ende > ',trim(modellverzeichnis),'lauf'
         if(errcode .ne. 0)call qerror('fortschritt systemaufruf cat lauf')
         call system(systemaufruf,system_error)
         if(system_error.ne.0)then
     ...
     &                         ,' fertig" < ',trim(modellverzeichnis),'lauf'
            if(errcode .ne. 0)call qerror('fortschritt systemaufruf mail fertig')
         ...
         if(errcode .ne. 0)call qerror('fortschritt systemaufruf stat codesource')
         call system(trim(systemaufruf),system_error)
         if(system_error.ne.0) then
     ...
     &                        ,adjustl(trim(modellverzeichnis)),' >/dev/null 2>/dev/null'
            if(errcode .ne. 0)call qerror('fortschritt systemaufruf cp  codesource')
            call system(trim(systemaufruf),system_error)
     ...
     &                                 ,trim(adjustl(codesource))," schlug fehl."
         ...
         if(errcode .ne. 0)call qerror('fortschritt: systemaufruf qusave modellverzeichnis fehlgeschlagen')
         ...
         if(errcode .ne. 0)call qerror('fortschritt systemaufruf ')
         call system(trim(systemaufruf),system_error)
         if(system_error.eq.0) then
         ...
         if(nur_temp)print*,'##### nur Temperatur-Simulation (itemp.eq.1 in EREIGG.txt) #####'
