...
!> arrays to read stored variables from .nc files, each process its part
      ...
      call MPI_Bcast(nst,1,MPI_INT,0,mpi_komm_welt,ierror)
      call MPI_Bcast(nin,1,MPI_INT,0,mpi_komm_welt,ierror)
      call mpi_barrier (mpi_komm_welt, ierror)

      if(nst.ne.nst_prev)then ! proceed to next stack
         if(meinrang.eq.0)print*,"get_schism_step proceeds to next stack ",nst,ncid
      ...
      do j=1,ndims
         ...
         call check_err(iret)
         if(iret.ne. 0) print*,meinrang,j," get_schism_step nf90_Inquire_Dimension failed iret=",iret
     ...
     !!!!! elev -> p
      ...
      call mpi_barrier (mpi_komm_welt, ierror)!#! 
      if(dlength(dimids(1)).gt.maxstack)call qerror("elev:dlength(dimids(1)).gt.maxstack")
      !! initialize
      do j=1,maxstack 
         var_p(j)=666.666
      end do ! all j
      ...
      call check_err(iret)
      if(iret.ne. 0) print*,meinrang," get_schism_step nf90_get_var elev failed iret=",iret
      ...
      call MPI_Gather(var_p, maxstack, MPI_FLOAT, var_g, maxstack, MPI_FLOAT, 0, mpi_komm_welt, ierror)
      if(ierror.ne.0)then
         write(fehler,*)"get_schism_step MPI_Gather(var_p elev failed : ", ierror
         call qerror(fehler)
      end if 
      call mpi_barrier (mpi_komm_welt, ierror)
      ...
      call check_err( iret )
      if(iret.ne. 0)call qerror("get_schism_step nf90_inquire_variable dahv failed")
      if(dlength(dimids(2)).gt.maxstack)call qerror("dahv:dlength(dimids(2)).gt.maxstack")
      !! initialize
      do j=1,maxstack 
         var_p(j)=666.666
      end do ! all j
      ...
      if(iret.ne. 0) print*,meinrang," get_schism_step nf90_get_var dahv1 failed iret=",iret
      call check_err(iret)
      ...
      call MPI_Gather(var_p, maxstack, MPI_FLOAT, var_g, maxstack, MPI_FLOAT, 0, mpi_komm_welt, ierror)
      if(ierror.ne.0)then
         write(fehler,*)"get_schism_step MPI_Gather(var_p dahv1 failed : ", ierror
         call qerror(fehler)
      end if 
      call mpi_barrier (mpi_komm_welt, ierror)
      !! recombine into global numbers
      ...
      count3 = (/1, npa, 1 /) ! nodenumber second dimension
      iret=  nf90_get_var(ncid, varid, var_p(1:npa), start3, count3 )
      if(iret.ne. 0) print*,meinrang," get_schism_step nf90_get_var dahv2 failed iret=",iret
      call check_err(iret)
      ...
      call MPI_Gather(var_p, maxstack, MPI_FLOAT, var_g, maxstack, MPI_FLOAT, 0, mpi_komm_welt, ierror)
      if(ierror.ne.0)then
         write(fehler,*)"get_schism_step MPI_Gather(var_p dahv2 failed : ", ierror
         call qerror(fehler)
      end if 
      call mpi_barrier (mpi_komm_welt, ierror)
      !! recombine into global numbers
      ...
      iret= nf90_inquire_variable(ncid,varid,vname(varid),vxtype(varid),vndims(varid),dimids, nAtts)
      call check_err( iret )
      if(iret.ne. 0)call qerror("get_schism_step nf90_inquire_variable hvel_side failed")
      !! initialize
      var_p(:)=666.666
      if(meinrang.eq. 0)then
         var_g(:)=777.777
	  ...
	  do i=1,nvrt
         ...
         if(iret.ne. 0) print*,meinrang," get_schism_step nf90_get_var hvel_side failed iret=",iret
         call check_err(iret)
         ...
         call mpi_barrier (mpi_komm_welt, ierror)
         call MPI_Gather(var1_p, maxstack, MPI_FLOAT, var1_g, maxstack, MPI_FLOAT, 0, mpi_komm_welt, ierror)
         call MPI_Gather(var2_p, maxstack, MPI_FLOAT, var2_g, maxstack, MPI_FLOAT, 0, mpi_komm_welt, ierror)
	  end do ! all i levels
      ...
      if(meinrang.eq. 0) var_g(:)=777.777
      var_p(:)=0.0
      call mpi_barrier (mpi_komm_welt, ierror)
 ...
 goto 7777 
      ...
      if(iret.ne. 0)then !no temp?
         ...
         iret= nf90_inquire_variable(ncid,varid,vname(varid),vxtype(varid),vndims(varid),dimids, nAtts)
         call check_err( iret )
         if(iret.ne. 0) print*,meinrang," get_schism_step nf90_inquire_variable temp failed iret=",iret
         if(dlength(dimids(2)).gt.maxstack)call qerror("temp:dlength(dimids(2)).gt.maxstack")
         !! initialize
         do j=1,maxstack 
            var_p(j)=666.666
         end do ! all j
      ...
      call MPI_Gather(var_p, maxstack, MPI_FLOAT, var_g, maxstack, MPI_FLOAT, 0, mpi_komm_welt, ierror)
      if(ierror.ne.0)then
         write(fehler,*)"get_schism_step MPI_Gather(var_p temp failed : ", ierror
         call qerror(fehler)
      end if 
      call mpi_barrier (mpi_komm_welt, ierror)
      !! recombine into global numbers
      ...
      if(iret.ne. 0)then ! salt?
         ...
         iret= nf90_inquire_variable(ncid,varid,vname(varid),vxtype(varid),vndims(varid),dimids, nAtts)
         call check_err( iret )
         if(iret.ne. 0) print*,meinrang," get_schism_step nf90_inquire_variable salt failed iret=",iret
         if(dlength(dimids(2)).gt.maxstack)call qerror("salt:dlength(dimids(2)).gt.maxstack")
         !! initialize
         do j=1,maxstack 
            var_p(j)=666.666
         end do ! all j
      ...
      call MPI_Gather(var_p, maxstack, MPI_FLOAT, var_g, maxstack, MPI_FLOAT, 0, mpi_komm_welt, ierror)
      if(ierror.ne.0)then
         write(fehler,*)"get_schism_step MPI_Gather(var_p salt failed : ", ierror
         call qerror(fehler)
      end if 
      call mpi_barrier (mpi_komm_welt, ierror)
      !! recombine into global numbers
      ...
      if(iret.ne. 0)then ! with age1 ?
         ...
         iret= nf90_inquire_variable(ncid,varid,vname(varid),vxtype(varid),vndims(varid),dimids, nAtts)
         call check_err( iret )
         if(iret.ne. 0) print*,meinrang," get_schism_step nf90_inquire_variable AGE_1 failed iret=",iret
         if(dlength(dimids(2)).gt.maxstack)call qerror("AGE_1:dlength(dimids(2)).gt.maxstack")
         !! initialize
         do j=1,maxstack 
            var_p(j)=666.666
         end do ! all j
         ...
         iret=  nf90_get_var(ncid, varid, var_p(1:npa), start3, count3 )
         call check_err(iret)
         if(iret.ne. 0) print*,meinrang," get_schism_step nf90_get_var AGE_1 failed iret=",iret
      end if ! with age
      call mpi_barrier (mpi_komm_welt, ierror)
      ! gather var_p into var_g
      call MPI_Gather(var_p, maxstack, MPI_FLOAT, var_g, maxstack, MPI_FLOAT, 0, mpi_komm_welt, ierror)
      if(ierror.ne.0)then
         write(fehler,*)"get_schism_step MPI_Gather(var_p AGE_1 failed : ", ierror
         call qerror(fehler)
      end if 
      call mpi_barrier (mpi_komm_welt, ierror)
      !! recombine into global numbers
 ...
 7777 continue
	  ...
	  if(meinrang.eq.0)then
         ...
         if(istat.ne.0) call qerror("deallocate var_g( failed")
      endif
      deallocate(var_p,var1_p,var2_p,stat = istat)
      if(istat.ne.0) call qerror("deallocate var_p( failed")
