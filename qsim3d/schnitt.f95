...
subroutine flux_casu(deltax,d1,d2,u1,u2,c1,c2,wsp1,wsp2,v1,v2,la,flae,vox,pox,kix,masx)
   ...
   if ( ((d1 > 0.0) .and. (d2 >= 0.0)) .or. ((d1 >= 0.0).and.(d2 > 0.0)) )fall = 1 ! Kante VOLL nass
   if ((d1 <= 0.0) .and. (d2 <= 0.0))fall = 2 ! NIX Kante ganz trocken
   if ((d1 < 0.0) .and. (d2 > 0.0))fall = 3 ! bottom1-trocken, top2-nass
   if ((d1 > 0.0) .and. (d2 < 0.0))fall = 4 ! bottom1-nass, top2-trocken
   if (fall == 0) call qerror('Fall-Unterscheidung in flux() fehlgeschlagen')
   select case (fall)
      case (1) ! VOLL
      ...
      case(3) ! bottom1-trocken-negativ, top2-nass-positiv
         ...
         x_kreuz = d1-d2
         if (x_kreuz == 0.0)call qerror("x_kreuz == 0.0,flux,case (3)")
      ...
      case(4) ! bottom1-nass-positiv, top2-trocken-negativ
         ...
         x_kreuz = d1-d2
         if (x_kreuz == 0.0)call qerror("x_kreuz == 0.0,flux,case (4)")
         x_kreuz = d1/x_kreuz !! relative Lage Uferpunkt (Kreuzung Sohle-Wasserspiegel) ab 1
...
logical function querschnitt_lesen()
   ...
   do i = 1,anzahl_quer
      ...
      if (querschnitt(i)%schnittlinie%anzkanten <= 1)call qerror("Quer-Schnitte mit weniger als 2 Knoten machen keinen Sinn!")
   ...
   !!!!!!!!!!!!!!!!!!! prüfen !!!!!!!!!!!!!!!!
   do i = 1,anzahl_quer !! alle i Querschnitte
      if ( knoten_rand(querschnitt(i)%schnittlinie%knoten(1)) <= 0)  &
          print*,"Erster Knoten in Querschnitt ",i," kein Randknoten"
      if ( knoten_rand(querschnitt(i)%schnittlinie%knoten(querschnitt(i)%schnittlinie%anzkanten+1)) <= 0)  &
          print*,"Letzter Knoten in Querschnitt ",i," kein Randknoten"
      if (querschnitt(i)%schnittlinie%knoten(1) == querschnitt(i)%schnittlinie%knoten(querschnitt(i)%schnittlinie%anzkanten+1))  &
