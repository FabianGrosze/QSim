...
if(meinrang.eq.0)then !! nur prozessor 0
      ...
      do j=1,number_plankt_point ! nur bei casu=knotenanzahl2D
         ...
         if( (RB_zaehl.gt. 0).and.(RB_zaehl.lt. 100000) )then !! Alle Knoten, deren RB's bedient werden:
         if(j .eq. kontrollknoten)print*,'Konrollstelle #',j,' ist Rand mit RB_zaehl=',RB_zaehl
         if(inflow(j))then !! alle Zufluss-Knoten
            if(j .eq. kontrollknoten)print*,'Konrollstelle #',j,' ist inflow Rand mit ',inflow(j)

            call randwert_planctonic(j,RB_zaehl,einmalig)

            if(j .eq. kontrollknoten)  &
...
!!  \anchor jjj wahrscheinlich ein Zähler (def. in randbedingungen; referenziert in N subroutine!?)
      ...
      if(meinrang.gt.0)then !! nur prozessor 0
         write(fehler,*)' 789 SUBROUTINE randwert_planctonic darf nur auf prozess 0'
         call qerror(fehler) 
      endif

         if(jjj.gt.number_plankt_point)call qerror('randwert_planctonic: jjj.gt.number_plankt_point')
         nk=(jjj-1)*number_plankt_vari
         if(nk+number_plankt_vari.gt.number_plankt_vari*number_plankt_point)then
            ...
            if(zaehl.gt.0)then !! Randbedingung zu dieser Nummer vorhanden
...
!     &                                           ," OCSB=",planktonic_variable(18+nk)
                  ...
                  if(nur_alter.and.(wie_altern.eq.2))call alter_rand(jjj)
...
!! \n\n aus randbedingungen.f95 , zurück: \ref zuflussranddaten
      ...
      integer :: jjj, nk, i, l
         ...
         if(jjj.gt.number_plankt_point)call qerror('tiefenprofil: jjj.gt.number_plankt_point')
         nk=(jjj-1)*number_plankt_vari
         if(nk+number_plankt_vari.gt.number_plankt_vari*number_plankt_point)then
...
!     if(j.eq.1)print*,'randbedingungen_ergaenzen'
      ...
      kontroll=.false.
      if(j .eq. kontrollknoten)kontroll=.true.
         ...
         do k=1,anzrawe !alle Rand-Variablen
            ...
            do j=1,rabe(n)%anz_rb ! alle Zeitpunkte dieser RB
               ...
               if(randwert_gueltig(wert,k))then
                  if(t.ge.rabe(n)%punkt(j)%zeit_sek)then ! letzter gültiger Wert vorher
                     vor_da=.true.
                     wert_vor=wert
            ...
            if(vor_da .and. nach_da)then
               ...
               if((k.eq.22).and.(kontrollknoten.gt. 0)) &
            ...
            if(vor_da .and. (.not.nach_da))then
               ...
               if(k.eq.22)print*,'Rand',n,' t,zeit_vor,wert_vor,rabe(n)%wert_jetzt(k)='  &
...
! 343 read(ion,*, iostat = read_error,end=341)nr,idumm,idumm,anzi
  343 continue
      ...
      if(read_error.ne.0) then
         print*,trim(ctext)
         write(fehler,*)' 343 ereigg_Randbedingungen_lesen() read_error.ne.0 ianz_rb=', ianz_rb
         call qerror(fehler)
      endif
      !print*,'RB; nr,anzi=',nr,anzi
      if(min_nr.gt.nr) min_nr=nr                                               
      if(max_rand_nr.lt.nr) max_rand_nr=nr                                               
      ianz_rb=ianz_rb+1
      if(anzi.gt.anzmax)anzmax=anzi
      if(anzi.eq.0)goto 343 
...
!
      ...
      if((max_rand_nr.le.0).or.(max_rand_nr.gt. 9999)) then
...
!    
      ...
      do n=1,ianz_rb !! alle Randbedingungsblöcke nochmal lesen
         ...
         if(read_error.ne.0)then
            write(fehler,*)' 344 read_error.ne.0 ereigg_Randbedingungen_lesen()'
            call qerror(fehler) 
         endif
    ...
    &          , rabe(n)%tagesmittelwert_flag 
      ...
      do i=1,ianz_rb
         do n=i+1,ianz_rb
            if( rabe(i)%nr_rb.eq.rabe(n)%nr_rb)then
...
!
   ...
   case(1) ! casu-transinfo                                           
      ...
      do j=1,knotenanzahl2D !! max Randnummer ermitteln:
         if(knoten_rand(j).gt. 0)then !! Randknoten
            if (knoten_rand(j).gt.maxrandnr)maxrandnr=knoten_rand(j)
         end if ! Randknoten
      end do ! alle j Knoten
      ...
      do j=1,knotenanzahl2D !! vorhandene Randummern durchzählen.
         if(knoten_rand(j).gt. 0)nr_vorhanden(knoten_rand(j))=nr_vorhanden(knoten_rand(j)) + 1
      end do ! alle j Knoten
      do j=1,maxrandnr !! alle Randnummern ausgeben:
         if(nr_vorhanden(j).gt. 0)then
      ...
      do j=1,maxrandnr !! Randbedingung für alle Randnummern vorhanden?
         rb_vorkommen(j)=0
         do n=1,ianz_rb !! alle Randbedingungen
            if(j.eq.rabe(n)%nr_rb)then
               print*,'Randnummer mit dem Zähler',n,' bedient die Randbedingung mit der Nummer ',rabe(n)%nr_rb
               rb_vorkommen(j)=rb_vorkommen(j) + 1
            end if
         end do !! alle n Randbedingungen
         if(rb_vorkommen(j).lt.1)print*,'### Warnung ###: für Randnummer ',j,' wurde keine Randbedingung vorgegeben.'
         if(rb_vorkommen(j).gt.1)then
      ...
      if(knoten_rand(j).gt. 0)then !! falls Randknoten 
         if(rb_vorkommen(knoten_rand(j)).eq.0)then !! nicht bediente Randnummern ausrangieren.
            knoten_rand(j)=999999
         else
            nini=-7
            do n=1,ianz_rb !! alle Randbedingungen
               if(knoten_rand(j).eq.rabe(n)%nr_rb)then
                  nini=n
               end if
            end do !! alle n Randbedingungen
            knoten_rand(j)=nini
         ...
         if(j.eq.kontrollknoten)then
            ...
            if(knoten_rand(j).le.ianz_rb)then
   ...
   case(2) ! Untrim² netCDF
      ...
      maxrandnr=0
      do j=1,n_elemente
         if(element_rand(j).gt.maxrandnr) maxrandnr=element_rand(j)
         if( (element_rand(j).lt.min_nr).and.(element_rand(j).gt. 0) )min_nr=element_rand(j)
         rb_vorhanden=.false.
         do n=1,ianz_rb !! alle Randbedingungen
            if(element_rand(j).eq.rabe(n)%nr_rb)rb_vorhanden=.true.
         end do !! alle n Randbedingungen
         if((.not.rb_vorhanden).and.(element_rand(j).ne. 0))  &
     ...
     &            ,element_rand(j),' zugehörige Randbedingung fehlt'
      ...
      do j=1,n_elemente !! vorhandene Randummern durchzählen.
         if(element_rand(j).gt. 0)nr_vorhanden(element_rand(j))=nr_vorhanden(element_rand(j)) + 1
      ...
      do n=1,ianz_rb !! alle Randbedingungen
          if(rabe(n)%nr_rb.lt.min_nr)  &
     &       print*,'ereigg_Randbedingungen_lesen Untrim: ',n,'-ter Rand mit nr=',rabe(n)%nr_rb,' unbenutzt'
          if(rabe(n)%nr_rb.gt.maxrandnr)  &
     &       print*,'ereigg_Randbedingungen_lesen Untrim: ',n,'-ter Rand mit nr=',rabe(n)%nr_rb,' unbenutzt'
      end do !! alle n Randbedingungen

      !! Randnummer in Randzähler umwandeln. 
      do j=1,n_elemente
      if(element_rand(j).gt. 0)then !! falls Rand
         if(rb_vorkommen(element_rand(j)).eq.0)then !! nicht bediente Randnummern ausrangieren.
            element_rand(j)=999999
         else
            nini=-7
            do n=1,ianz_rb !! alle Randbedingungen
               if(element_rand(j).eq.rabe(n)%nr_rb)then
                  nini=n
               end if
            end do !! alle n Randbedingungen
            element_rand(j)=nini
         ...
         if(j.eq.kontrollknoten)then
            ...
            if(element_rand(j).le.ianz_rb)then
   ...
   case(3) ! SCHISM
      ...
      do j=1,maxrandnr !! Randbedingung für alle Randnummern vorhanden?
         ...
         if(rb_vorkommen(j).lt.1)print*,'### Warnung ###: für Randnummer ',j,' wurde keine Randbedingung vorgegeben. -SCHISM'
         if(rb_vorkommen(j).gt.1)then
...
!!
      ...
      if(meinrang.ne.0)then ! prozess 0 only
...
!!
      ...
      do n=1,ianz_rb !! alle Randbedingungen
         ...
         do i=1, rabe(n)%randlinie%anzkanten ! alle i Kanten aus randlinie n
            ...
            if (nbot.le.0)then
               write(fehler,*)nbot,'=nbot=rabe(',n,')%randlinie%kante(',i,')%bottom  .le.0 '
               call qerror(fehler)
            endif
...
!! \n\n aus randbedingungen.f95 , zurück: \ref zuflussranddaten
      ...
      do n=1,ianz_rb !! alle Randbedingungen
         ...
         do j=1,n_elemente ! alle j Elemente
            ...
            do k=1,cornernumber(j) ! 3 oder 4
               if( knoten_rand(elementnodes(j,k)).eq.n) nexi=nexi+1
            end do ! alle k knoten im Element j
            if(nexi.eq.2)then ! Normalfall, Element hat eine Kante auf dem Rand
               anzel=anzel+1
               rabe(n)%randlinie%kante(anzel)%element=j
               top=.true.
               do k=1,cornernumber(j) ! alle 3 oder 4 Knoten im Element j
               if( knoten_rand(elementnodes(j,k)).eq.n) then ! Randknoten
                  ...
                  if(dreidrin.gt.0)vierdrin=elementnodes(j,k)
                  if(vierdrin.eq.0)dreidrin=elementnodes(j,k)
               end if ! Randknoten
               ...
               if(einwaerts.ge.0.0)then ! Normalenvektor auswärts drehen + top-bottom tauschen
            ...
            if(nexi.ge.3)then ! unerwünschter Sonderfall
    ...
    ! &                      , knoten_y(rabe(n)%randlinie%kante(anzel)%top)
     ...
     &         ," Kanten und ist ",lang," m lang"
         if(anzel.eq.rabe(n)%randlinie%anzkanten) then
            print*,"Rand mit Nummer=",rabe(n)%nr_rb,"regulär"
         else
            if (anzel.eq.rabe(n)%randlinie%anzkanten+1)then
