      subroutine v_konz(tempwz,hcdif1,hcdt,dtemp,nkzs,xD,ior,vo2z,hJO2,dO2o2D,vz1,vNH4z,vNO2z,vNO3z                     &                                      
...
!  ...Subroutine zur Ermittlung der vertikalen Konzentrationsverteilung 
!  ...mit Hilfe eines impliziten Differenzenverfahren                   
...
!....Crank-Nicolson                                                     
      do nkz = 1,nkzs(ior) 
        ...
        if(nkz>1)then 
          ...
          if(vo2zt.lt.0.0)vo2zt = (vo2z(nkz,ior)/(vo2z(nkz,ior)+abs(vz1t)))*vo2z(nkz,ior)                                        
          vo2z(nkz,ior) = vo2zt
        endif 
                                                                       
        tempzt = tempwz(nkz,ior)+dtemp(nkz,ior)*hcdt 
        if(tempzt.lt.0.0)tempzt = (tempwz(nkz,ior)/(tempwz(nkz,ior)+abs(dtemp(nkz,ior)*hcdt)))*tempwz(nkz,ior)                       
        tempwz(nkz,ior) = tempzt 
     enddo

      vo2zt = vo2z(1,ior)+delO2       ! oberflächenschicht, physikalischer Sauerstoffein/austrag 
      if(vo2zt.lt.0.0)vo2zt = (vo2z(1,ior)/(vo2z(1,ior)+abs(delo2)))*vo2z(1,ior)                                         
      vo2z(1,ior) = vo2zt 

! Sohlschichten
   
      vo2zt = vo2z(nkzs(ior),ior)-dBetO2 
      if(vo2zt.lt.0.0)vo2zt = (vo2z(nkzs(ior),ior)/(vo2z(nkzs(ior),ior)+abs(dBetO2)))*vo2z(nkzs(ior),ior)                                
      ...
      if(vNO3zt.lt.0.0)vNO3zt = (vNO3z(nkzs(ior),ior)/(vNO3z(nkzs(ior),ior)+abs(dBetN3)))*vNO3z(nkzs(ior),ior)                               
...
!   enddo                      ! Ende Schleife
      ...
      j = nkzs(ior) 
      do nkz=1,nkzs(ior) 
        xD(j) = 0.9998395+0.000067914*Tempwz(nkz,ior)-0.0000090894*Tempwz(nkz,ior)**2            &
               +0.00000010171*Tempwz(nkz,ior)**3-0.0000000012846*Tempwz(nkz,ior)**4              &
               +0.000000000011592*Tempwz(nkz,ior)**5-5.0125e-14*Tempwz(nkz,ior)**6                                    
        xD(j) = xD(j)*1000. 
        j = j-1 
      enddo 
