...
!---------------------------------------------------------------------------------------
  ...
  subroutine Transportz(anze,deltat,izeits,isub_dt,isub_dt_Mac,dtmin_Mac,hvmitt,elen,flag          &
            ,tempwz,vnh4z,vno2z,vno3z,vo2z,gelPz,Siz,akiz,agrz                                     &
            ,ablz,chlaz,hgesPz,hgesNz,nkzs,dH2D,i2Ds,iwsim,mstr                                    &
            ,htempz,ho2z,hnh4z,hno2z,hno3z,hgelPz,hSiz,hQ_NKz,hQ_NBz,hQ_NGz,hakiz,hagrz            &
            ,hablz,hchlaz,hchlkz,hchlgz,hchlbz,hCChlkz,hCChlbz,hCChlgz,iflRi,dl,iMAC,Uvert,tflie   &
            ,jpoin1,itags,monats,iwied,uhrz,iverfahren,azStrs,ianze_max,nkztot_max,Qmx_NK          &
            ,Qmx_NB,Qmx_NG,mtracer)                                                           
...
!$omp parallel do
        do ior = 1,anze+1
          if(nkzs(ior).gt.nkzmax)nkzmax = nkzs(ior) 
        enddo 
!$OMP END PARALLEL Do
        ...
        call sys_gitterTrans(mstr,anze,nkzs,nkzmax,dH2D,tempwz,vo2z,vnh4z &
                             ,vno2z,vno3z,gelPz,Siz,akiz,agrz,ablz,chlaz,hchlkz,hchlgz,hchlbz,hCChlkz,hCChlbz,hCChlgz   &
                             ,hgesPz,hgesNz,hQ_NKz,hQ_NBz,hQ_NGz,Uvert,i_point,itags,monats,uhrz,azStrs)
...
!    endif  ! Ende Programmteil "Strömung"
      ...
      do ior = 1,anze+1 
        ...
        if(ktrans.eq.2)vnh4z(nkz,ior) = U(ior) 
        if(ktrans.eq.3)vno2z(nkz,ior) = U(ior) 
        if(ktrans.eq.4)vno3z(nkz,ior) = U(ior) 
        if(ktrans.eq.5)vo2z(nkz,ior) = U(ior) 
        if(ktrans.eq.6)gelPz(nkz,ior) = U(ior) 
        if(ktrans.eq.7)Siz(nkz,ior) = U(ior)           
        if(ktrans.eq.8)akiz(nkz,ior) = U(ior) 
        if(ktrans.eq.9)agrz(nkz,ior) = U(ior) 
        if(ktrans.eq.10)ablz(nkz,ior) = U(ior) 
        if(ktrans.eq.11)chlaz(nkz,ior) = U(ior) 
        if(ktrans.eq.12)hchlkz(mstr,nkz,ior) = U(ior) 
        if(ktrans.eq.13)hchlgz(mstr,nkz,ior) = U(ior) 
        if(ktrans.eq.14)hchlbz(mstr,nkz,ior) = U(ior) 
        if(ktrans.eq.15)hgesPz(mstr,nkz,ior) = U(ior) 
        if(ktrans.eq.16)hgesNz(mstr,nkz,ior) = U(ior) 
        if(ktrans.eq.17)hQ_NKz(mstr,nkz,ior) = min(Qmx_NK,U(ior)) 
        if(ktrans.eq.18)hQ_NBz(mstr,nkz,ior) = min(Qmx_NB,U(ior)) 
        if(ktrans.eq.19)hQ_NGz(mstr,nkz,ior) = min(Qmx_NG,U(ior)) 
        if(ktrans.eq.20)hCChlkz(mstr,nkz,ior) = U(ior) 
        if(ktrans.eq.21)hCChlbz(mstr,nkz,ior) = U(ior)
        if(ktrans.eq.22)hCChlgz(mstr,nkz,ior) = U(ior) 

        if(itime.eq.izeits.and.ktrans.eq.22)nkzs_1(mstr,ior) = nkzs(ior) 
      enddo 
