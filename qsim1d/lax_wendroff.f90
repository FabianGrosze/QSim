subroutine lax_wendroff(U, vmitt, Uvert, DX, DT, NX, flag, nkz, ktrans, isgn, iwahld)
   ...
   ! --- local variable ---
   integer                  :: n, j, jj, i
   real                     :: s3, s2, s1, hcon, cour_mit
   real, dimension(1000) :: cour, U_neu, phi
   ...
   do i = j,nx+1-jj
      ...
      ! minmod-Limiter
      ! downwind
      ! phi(i) = (S3-S2) * max(0.0, min(1.0,hcon))

      ! superbee-Limiter
      phi(i) = (S3-S2) * max(0.0, min(1.0,2.*hcon), min(2.,hcon))
   enddo

   ! Beginn Gitterschleife
   do i = j,nx+1-jj
      if (isgn(i,nkz) == 1) then
         n = 1
         S1 = U(i-1)
      ...
      if (flag(i) == 4 .or. isgn(i,nkz) == 0) then
         ...
         U_neu(i) = S2                                          &
                  - (cour_mit * (S2-S1))                        &
                  - 0.5 * cour_mit * (1.-cour_mit) * Phi(i)     &
                  + 0.5 * cour_mit * (1.-cour_mit) * Phi(i-n)
      endif
   ...
   if (NX == 1) then 
      ! Falls der Strang nur aus einem Knoten besteht
      U_neu(1) = U(1)
      U_neu(NX+1) = U_neu(1)
   else
