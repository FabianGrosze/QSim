 subroutine WEHR(wehrh,wehrb,ho2,hQaus,O2zt,htempw,ho2_z,ho2z_z,hlf,hpw,hmw,hph,hph_z,iph                                &                                  
          ...
          ,Q_NBzt, hQ_NBz_z,Q_NGzt, hQ_NGz_z,dH2D,ESTRNR,kanz,inkzmx                                                     &
          ,iSta,nstr,istr,jnkz,iflRi,jlWO2,CChlkzt,hCChlkz_z,CChlbzt,hCChlbz_z,CChlgzt,hCChlgz_z                         &
          ,janzWS,janzWt,hnkzs,mwehr,mstr,WSP_UW,WSP_OW,iB,azStrs)                                                           



!                                                                       
!                                                                       
!    Ein Programm zur Ermittlung des Einflusses von Wehren auf den      
!    Stoffhaushalt 

!                                                         
!                                                                       
!....Co2austrag müss noch geändert werden!!!                            
!                                                                       
...
!                                                                       
      ...
      if(wehrh(ESTRNR(istr,nstr))<=0.0)then                   ! falls keine Wehrhoehe eingegeben wird sie aus der Wasserspiegeldiff. ermittelt
        Fallhoehe = wsp_OW(ESTRNR(istr,nstr)) - WSP_UW(mstr)
          else
            Fallhoehe = wehrh(ESTRNR(istr,nstr))              ! eingegebene Wehrhöhe oder berechnete
                                                              ! (in subroutine wehrles) fiktive Wehrhöhe 
       endif

       if(jlwo2(ESTRNR(istr,nstr))==0)then            ! keine Angaben ueber Anzahl der Wehrfelder und gezogene Wehrklappen; keine wehro2.txt. Wehrüberfall soll beruecksichtigt werden!!  
      ...
      if(jlWO2(ESTRNR(istr,nstr))==2)then 
        ...
        hconWB = hcon1/hcon2                        ! Verhältnis von gezogenen Wehrklappen zur Gesamtzahl an Wehrklappen
      endif 
