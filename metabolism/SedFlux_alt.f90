  subroutine sedflux(tiefe,vmitt,rau,sedAlg_MQ,hSedOM,hw2,hBedGS,hsedvvert,hdKorn,vO2,vNO3,vNH4,gelP,Tempw,anze,mstr          &
                   ...
                   ,Q_PB,pl0,nl0,Si,hSised,hJSi,aki,agr,abl,Chlaki,Chlagr,Chlabl,hFluN3,ilang,azStrs,iwied,yNmax1e,Stks1e     &
...
!+++Berechnung des Diffusionskoeffizienten in der aeroben und anaeroben Schicht, DiffK1, DIFFK2
  call Sed_DiffK(tiefe,vmitt,rau,H1,H2,hdKorn,DiffK1,DiffK2,DifKP1,DifKP2,poro1,poro2,vvert,vvert1,vvert2      &
                 ,mstr,ior,itags,monats,uhrz,azStrs)

  if(it==1)s = Dp/H1

  KL12 = DiffK2*ThtaD**(Temps-20.)/(H2/2.)
...
!
 s = SOD/O20
 H1 = DiffK1*ThtaD**(Temps-20.)/s ! Neuberechnung der Dicke der aeroben Schicht
...
!  Denit(1) = sqrt(KappN3*ThtaN3**(Temps-20.)*s)        ! ich
   Denit(1) = KappN3**2*ThtaN3**(Temps-20.)/s*hconD1           ! nach DiToro
...
!
         ...
         POCsed = (POCsed1+POCsed1)*(1./tflie)
     ...
     Enddo !Ende Schleife für Berücksichtigung des Bedeckungsgrads
