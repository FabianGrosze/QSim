...
!         wrid nur bei den Nährstoffen berücksichtigt.                  
!                                                                       
      zqz10 = 3.1 
      ztmax = 31. 
      ztopt = 28. 
...
!                                                                       
      ...
      if(azStr==1)then
        drrt = drrt+tflie 
        itime_hoch = 1          ! Schalter für Hochzählen der div. Zeiten
      endif      
...
!                                                                       
      if(coroI(ior).eq.0.0.and.coroIs(ior).eq.0.0)then 
        ...
        fcom = 1. 
           ...
           if((hconc1+hconc2).gt. 0.0)fcom = (hconc1*fco+hconc2*fcos)/(hconc1+hconc2) 
       endif
...
!                                                                       
  501 Filaki(ndr) = adrk(ndr) 
      filagr(ndr) = adrg(ndr) 
      filabl(ndr) = adrb(ndr) 
      if(aki(ior).gt. 0.0)vofkop(ndr) = (Filaki(ndr)/(aki(ior)*pki))*100. 
      goto 515 
!                                                                       
  502 Filaki(ndr) = adrk(ndr) 
      filagr(ndr) = adrg(ndr) 
      filabl(ndr) = adrb(ndr) 
      if(agr(ior).gt. 0.0)vofkop(ndr) = (Filagr(ndr)/(agr(ior)*pgr))*100. 
      goto 515 
!                                                                       
  503 Filaki(ndr) = adrk(ndr) 
      filagr(ndr) = adrg(ndr) 
      filabl(ndr) = adrb(ndr) 
      if(abl(ior).gt. 0.0)vofkop(ndr) = (Filabl(ndr)/(abl(ior)*pbl))*100. 
!                                                                       
  515 dchlg(ndr) = 0.0
      dchlk(ndr) = 0.0
      dchlb(ndr) = 0.0
...
!                                                                       
      ...
      spwmx =  0.0
      if((drrt.gt.drrt3).and.((laid1-drrt3).ne. 0.0))spwmx = flai*0.4/(0.5*(laid1-drrt3)) 
!                                                                       
...
!                                                                       
      ...
      fdrrt = 0.0 
      if(((drrt-drrt2)**2+(drrt-drrt1)**2).gt. 0.0)fdrrt = ((drrt-drrt1)**2)/((drrt-drrt2)**2+(drrt-drrt1)**2) 
      fdrrt = fdrrt*spwmx 
      goto 250 
  220 fdrrt = 0.0 
      if(((drrt-drrt2)**2+(drrt-drrt3)**2).gt. 0.0)fdrrt = ((drrt-drrt3)**2)/((drrt-drrt2)**2+(drrt-drrt3)**2) 
      fdrrt = fdrrt*spwmx 
      goto 250 
!                                                                       
!   zweite Kurve                                                        
!                                                                       
  221 drrtt = drrt-drrt3 
      ...
      fdrrt = 0.0 
      if(((drrtt-drrt22)**2+(drrtt-drrt11)**2).gt. 0.0)fdrrt = ((drrtt-drrt11)**2)/((drrtt-drrt22)**2+(drrtt-drrt11)**2) 
      fdrrt = fdrrt*spwmx 
      goto 250 
  225 fdrrt = 0.0 
      if(((drrtt-drrt22)**2+(drrtt-drrt33)**2).gt. 0.0)fdrrt = ((drrtt-drrt33)**2)/((drrtt-drrt22)**2+(drrtt-drrt33)**2) 
      fdrrt = fdrrt*spwmx 
!                                                                       
  250 continue 
      do 216 ndr=2,nndr 
...
!                                                                       
      ...
      dgwmue = 0.0
      if((lboem(ior)*elen(ior)).gt. 0.0)dgwmue = (dyc/(2.*lboem(ior)*elen(ior)))*0.52
      if((bsohlm(ior)*elen(ior)).gt. 0.0)dgwmue = dgwmue +(dycs/(bsohlm(ior)*elen(ior)))*0.52                              
...
!                                                                       
      ...
      spwmx =  0.0
      if((drft.gt.drrt3).and.((laid1-drrt3).ne. 0.0))spwmx = flai*0.4/(0.5*(laid1-drrt3)) 
!                                                                       
!                                                                       
!                                                                       
...
!                                                                       
      ...
      fdrrt = 0.0 
      if(((drft-drrt2)**2+(drft-drrt1)**2).gt. 0.0)fdrrt = ((drft-drrt1)**2)/((drft-drrt2)**2+(drft-drrt1)**2) 
      fdrrt = fdrrt*spwmx 
      goto 350 
  322 fdrrt = 0.0 
      if(((drft-drrt2)**2+(drft-drrt3)**2).gt. 0.0)fdrrt = ((drft-drrt3)**2)/((drft-drrt2)**2+(drft-drrt3)**2) 
      fdrrt = fdrrt*spwmx 
      goto 350 
!                                                                       
!   zweite Kurve                                                        
!                                                                       
  325 drftt = drft-drrt3 
      ...
      fdrrt = 0.0 
      if(((drftt-drrt22)**2+(drftt-drrt11)**2).gt. 0.0)fdrrt = ((drftt-drrt11)**2)/((drftt-drrt22)**2+(drftt-drrt11)**2) 
      fdrrt = fdrrt*spwmx 
      goto 350 
  320 fdrrt = 0.0 
      if(((drftt-drrt22)**2+(drftt-drrt11)**2).gt. 0.0)fdrrt = ((drftt-drrt33)**2)/((drftt-drrt22)**2+(drftt-drrt33)**2) 
      fdrrt = fdrrt*spwmx 
!                                                                       
!                                                                       
  350 continue 
...
!....Larvengewicht beim Festsetzen: 8.6e-8 gC; 8.6e-5 mgC               
  ...
  116 dfemue = 0.0
      if((tdpla.gt. 0.0).and.(tflie.gt. 0.0))dfemue = sgwmue(ior)/(tdpla*1./tflie)
...
!                                                                       
      ...
      dfemue = 0.0 
      dfmue = 0.0 
      if((zdrei(ior,2)+zdreis(ior,2)).gt. 0.0)then
...
!                                                                       
      ...
      dreisn = 0.0 
      if(gewdr(ior,ndr).gt. 0.0)dreisn = ((zdrei(ior,ndr)+zdreis(ior,ndr))*1000.)/gewdr(ior,ndr) 
      dreing = dreisn 
  219 if(ndr.eq.1.and.dlafes.gt.0.0)then 

      gewdts = 0.0
      if((dreisn+dlafes*vol*1000.).gt. 0.0)gewdts = (dreisn*gewdr(ior,1)+dlafes*vol*1000.*8.6e-5)            &
...
!                                                                       
      ddrein = 0.0 
      if(gewdr(ior,1).gt. 0.0)ddrein = ((zdrei(ior,1)+zdreis(ior,1))*1000.)/gewdr(ior,1) 
      dreing = dreisn+ddrein 
!                                                                       
      gewdr(ior,2) = 0.0 
      if((dreisn+ddrein).gt. 0.0)gewdr(ior,2) = (dreisn*gewdr(ior,2)+ddrein*gewdr(ior,1))          &
     &/(dreisn+ddrein)                                                  
!                                                                       
      gewdr(ior,1) = 0.0 
      zdrei(ior,1) = 0.0 
...
!nur ausschreiben                                                       
     ...
     &(zdrei(ior,ndr)+zdreis(ior,ndr)).eq.0.0)then                      
         hcond1 = 0.0 
         hcond2 = 0.0 
         if((zdrei(ior,2)+zdreis(ior,2)).gt. 0.0)hcond1 = zdrei(ior,2)/(zdrei(ior,2)+zdreis(ior,2)) 
         if((zdrei(ior,2)+zdreis(ior,2)).gt. 0.0)hcond2 = zdreis(ior,2)/(zdrei(ior,2)+zdreis(ior,2)) 
         goto 351 
      endif 
!                                                                       
...
!                                                                       
     ...
     &.and.ddrein.gt.0.0)then                                           
         hcond1 = 0.0
         hcond2 = 0.0 
         if((hcondb(1)+hconds(1)).gt. 0.0)hcond1 = hcondb(1)/(hcondb(1)+hconds(1)) 
         if((hcondb(1)+hconds(1)).gt. 0.0)hcond2 = hconds(1)/(hcondb(1)+hconds(1)) 
         goto 351 
      endif 
!                                                                       
      hcond1 = 0.0
      hcond2 = 0.0 
      if((hcondb(ndr)+hconds(ndr)).gt. 0.0)hcond1 = hcondb(ndr)/(hcondb(ndr)+hconds(ndr)) 
      if((hcondb(ndr)+hconds(ndr)).gt. 0.0)hcond2 = hconds(ndr)/(hcondb(ndr)+hconds(ndr)) 
!                                                                       
  ...
  217 continue 
      ...
      do 872 ndr = 1,nndr 
      zdrei(ior,ndr) = 0.0
      if((2.*lboem(ior)*elen(ior)).gt. 0.0)zdrei(ior,ndr) = zdrei(ior,ndr)/(2.*lboem(ior)*elen(ior)) 
      if(bsohlm(ior).le. 0.0)then 
      zdreis(ior,ndr) = 0.0 
      goto 872 
      endif 
      zdreis(ior,ndr) = 0.0 
      if((bsohlm(ior)*elen(ior)).gt. 0.0)zdreis(ior,ndr) = zdreis(ior,ndr)/(bsohlm(ior)*elen(ior))
  872 continue 
!                                                                       
  211 continue
      if((2.*lboem(ior)*elen(ior)).ne. 0.0)then
         dlmax(ior) = dlmax(ior)/(2.*lboem(ior)*elen(ior)) 
      endif 
      if( (bsohlm(ior)*elen(ior)) .ne. 0.0)then
         dlmaxs(ior) = dlmaxs(ior)/(bsohlm(ior)*elen(ior)) 
      endif 
!                                                                       
  111 continue 
