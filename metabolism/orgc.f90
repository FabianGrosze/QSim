...
!........
      ...
      vcb = 0.0                                                                 
      if(ocsb(ior).gt. 0.0) vcb = obsb(ior)/ocsb(ior)
      Cref = 0.0
      if(TOC_CSB.gt. 0.0) &
...
!.. Aenderung der Konz. an monomolekularen C-Verbindungen              
      ...
      do i=1,2 
        hyD(i) = 0.0
        if((CDt(i)+ksD(i)).gt. 0.0) hyD(i) = hymxD(i)*(CDt(i)/(CDt(i)+ksD(i))) 
        ddCM(i) = hyD(i)*ftemp*BAC(ior)*tflie 
        if(ddCM(i).gt.CDt(i))ddCM(i) = CDt(i) - 0.00001 
      enddo 
...
!     Bakterienwachstum                                                 
      ...
      upBAC = 0.0
      if((CMt+ksM).gt. 0.0) upBAC = upBACm*ftemp*(CMt/(CMt+ksM)) 
      dCM = BAC(ior)*(exp(upBAC*tflie)-1.) 

      if(dCM>CMt)then 
         upBAC = 0.0
         if(tflie.gt. 0.0) upBAC = (log(BAC(ior)+CMt-0.00001)-log(BAC(ior)))/tflie 
         dCM = BAC(ior)*(exp(upBAC*tflie)-1.) 
      endif
...
!.......Umrechnung auf g/(m3*d)                                         
      hconPf= 0.00                                          
      if(tiefe(ior).gt. 0.0) hconPf = pfl(ior)/(300.*tiefe(ior)) 
...
!     einfluss der Sedimentation                                        
      ...
      g = sqrt(9.81) 
      ust = 0.0
      if( (rau(ior).gt. 0.0) .and. (tiefe(ior).gt. 0.0) )   &
...
!
      ...
      fakCref = 0.0
      if(sumc.gt. 0.0) fakCref = (CP(1,ior)+CP(2,ior))/sumc
      sumc= (CP(1,ior)+CP(2,ior)+fakCref*Cref)
      if(sumc.gt. 0.0) CPt(1) = CPt(1)-ssdr(ior)*0.3*0.4*(CP(1,ior)/ sumc )                        
      if(sumc.gt. 0.0) CPt(2) = CPt(2)-ssdr(ior)*0.3*0.4*(CP(2,ior)/ sumc )                        
      if(sumc.gt. 0.0) Creft = Creft-ssdr(ior)*0.3*0.4*(cref/ sumc )                             
...
!  Grenzkonzentration 
      ...
      if(sumc.gt. 0.0) hc1 = hc1-(ssdr(ior)*0.3*0.4*(CP(2,ior)/ sumc ))                       
      ...
      if(sumc.gt. 0.0) hc2 = hc2-(ssdr(ior)*0.3*0.4*(CP(2,ior)/ sumc ))                       
                                                                       
      fbsgrt = 0.0
      if(hc1.gt. 0.0) fbsgrt = max(0.0,min(0.9,(hc2/hc1))) 
      ...
      if(sumc.gt. 0.0) hc1 = hc1-(ssdr(ior)*0.3*0.4*(cref/ sumc ))                            
      ...
      if(sumc.gt. 0.0) hc2 = hc2-(ssdr(ior)*0.3*0.4*(cref/ sumc ))                            
                                                                       
      frfgrt = 0.0 
      if(hc1.gt. 0.0) frfgrt = max(0.0,min(0.9,(hc2/hc1))) 
...
!                                                                       
      nl0(ior) = 0.0
      pl0(ior) = 0.0
      if((ocsbt.gt. 0.0).and.(TOC_CSB.gt. 0.0))then
