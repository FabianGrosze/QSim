...
!...Silikat (kein zellinterner Speicher)                                
    ...
    if(hcon.ne. 0.0) hcon = Siz(nkz,ior)/hcon ! Silikatgehlat der Zelle ist abhängig vom Silikatgehalt im Außenmedium
    Q_SKz(nkz) = Qmx_SK*hcon 
    up_Siz(nkz,ior) = akibrz(nkz,ior)-algakz(nkz,ior) 
    if(up_Siz(nkz,ior).ne. 0.0)up_Siz(nkz,ior) = Qmx_SK*hcon*(max(0.0,(dalgkz(nkz,ior)-algakz(nkz,ior))))/up_Siz(nkz,ior)
...
!...Silikat (kein zellinterner Speicher)
    hcon = Si(ior)+akkssi
    if(hcon.ne. 0.0) hcon = Si(ior)/hcon ! Silikatgehlat der Zelle ist abhängig vom Silikatgehalt im Außenmedium
    if(nkzs(ior)==1)Q_SKt = Qmx_SK*hcon 
    up_Si(ior) = akitbr(ior)-dalgak(ior)
    if(up_Si(ior).ne. 0.0)up_Si(ior) = Qmx_SK*hcon*(max(0.0,(dalgki(ior)-dalgak(ior))))/up_Si(ior) 
...
!..Annahme: sedimentiert wie lebende Biomasse                           
      ...
      hconoc = 0.0 
      if(aki(ior).gt.0.0)hconoc = sedalk(ior)/aki(ior) 
