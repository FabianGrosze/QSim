...
subroutine ph_kern(mws,pws,cas,lfs,tempws,vphs,vco2s                  &
   ...
   if (kontroll) then
      print*,'ph_kern oberflächenbelüftung, DCO2o,defco2,saetco2,MGCO2 = ' &
   ...
   if (cat < 0.0)cat = (cas/(cas+abs(delca)))*cas
   if (Lft < 0.0)Lft = (lfs/(lfs+abs(delLf)))*lfs
   if (mwt < 0.0)mwt = (mws/(mws+abs(delmw)))*mws
   cas = cat
   mws = mwt
   pws = pwt
   lfs = lft
