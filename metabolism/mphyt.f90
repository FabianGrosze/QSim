...
!                                                                       
   29 continue 
      ...
      do 111 ior=1,anze 
!                                                                       
      if(itstart.le.0.and.pflmax(ior).gt.0.0)then 
            ifehl = 17 
            ifhStr = mstr 
            goto 999 
      endif 
!                                                                       
      if(pflmax(ior).le.0.0)then 
      pfl(ior) = 0.0 
      goto 111 
      endif 
!                                                                       
...
!                                                                       
      ...
      po2r(ior) = 0.0 
      pfldalg(ior) = 0.0 
      if(pflmax(ior).eq.0.0)goto 111 
      if(obfli.eq.0.0)then 
      algip1 = 0.0 
      goto 50 
      endif 
!                                                                       
      ...
      if(tlip1.lt.0.01)tlip1 = 0.01 
      if(tlip1.gt.tiefe(ior))tlip1 = tiefe(ior) 
      algip1 = obfli*(1./(tlip1*extk(ior)))*(1.-exp(-extk(ior)*tlip1)) 
!                                                                       
   50 continue 
      if(nrs.ge.nrend.or.nrsi.ge.nrend)then 
      pfl(ior) = pflmin(ior) 
      goto 56 
      endif 
      if(nrs.le.nrstart)then 
      pfl(ior) = pflmin(ior) 
      goto 56 
      endif 
      if(obfli.eq.0.0)goto 56 
!                                                                       
...
!                                                                       
      if(pflmax(ior).gt.0.0.and.Pfl(ior).eq.0.0)pfl(ior) = pfln 
!                                                                       
      mgrmax = log(pfln1)-log(pfln) 
      mgrmax = mgrmax*24./(su-sa) 
      if(mgrmax.lt.0.0)then 
      mgrow = mgrmax 
      goto 59 
      endif 
