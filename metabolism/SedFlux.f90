...
!---------------------------------------------------------------------------------------
  ...
  subroutine sedflux(tiefe,vmitt,rau,sedAlg_MQ,hSedOM,hw2,hBedGS,hsedvvert,hdKorn,vO2,vNO3,vNH4,gelP,Tempw,anze,mstr          &
                   ...
                   ,obsb,ocsb,  kontroll ,jjj ) !!wy  
                    
!                                                            
!                                                            
!     Ein Programm zur Ermittlung der SoffFluxe aus dem Sediment
!                                                            
!     DiToro 2001; QUAL2K
!                                                            
!                                                            
...
! open(unit=601,file='sedflux.tst')
...
!      if(kontroll)then
!         print*,'sedflux: vor Sed_DiffK ',it,jjj
...
!      end if
  ...
  if(it==1)s = Dp/H1
      ...
      !if(kontroll)print*,'sedflux: KL12 = DiffK2*ThtaD**(Temps-20.)/(H2/2.)',DiffK2,ThtaD,Temps,H2
     ...
     Do i = 1,2 ! Summenbildung in der aeroben Schicht
       if(i==1)then
        ...
        if(H11<0.0)then
         ...
         !! Summation falsch??? der JPOC_neu und der  POCvert1 zweimal hinzuaddiert???
         Jc1 = Jc1 + KdiaPC(i)*fTPOM * POC1(i)*H11 + xK1DOC(i)*fTPOM*JPOC_neu(mstr,ior)*hcalphals*fakH + xK1DOC(i)*fTPOM * DOC1(i)*H1             &
                   + xK1DOC(i)*fTPOM * POCvert1(mstr,ior)* hcalphals
         JN1 = JN1 + KdiaPC(i)*fTPOM * PON1(i)*H11 + xK1DOC(i)*fTPOM*JPON_neu(mstr,ior)*hcalphals*fakH + xK1DOC(i)*fTPOM * DOC1(i)*nl0(ior)*H1    &
                   + xK1DOC(i)*fTPOM * POCvert1(mstr,ior)*nl0(ior)*hcalphals
         JP1 = JP1 + KdiaPC(i)*fTPOM * POP1(i)*H11 + xK1DOC(i)*fTPOM*JPOP_neu(mstr,ior)*hcalphals*fakH + xK1DOC(i)*fTPOM * DOC1(i)*pl0(ior)*H1    &
...
!
     Do i = 1,2 !Summenbildung in der anaeroben Schicht 
      ...
      Jc = Jc + KdiaPC(i)*fTPOM * POC2(i)*H2 + xK1DOC(i)*fTPOM * DOC2(i)*H2 + xK1DOC(2)*fTPOM *POCvert2(mstr,ior)
      JN = JN + KdiaPC(i)*fTPOM * PON2(i)*H2 + xK1DOC(i)*fTPOM * DOC2(i)*nl0(ior)*H2 + xK1DOC(2)*fTPOM                &
           * POCvert2(mstr,ior)*nl0(ior) 
      JP = JP + KdiaPC(i)*fTPOM * POP2(i)*H2 + xK1DOC(i)*fTPOM * DOC2(i)*pl0(ior)*H2 + xK1DOC(2)*fTPOM                &
           *POCvert2(mstr,ior)*pl0(ior)
     enddo 
...
!     Ammonium 
!##############
!      !if(kontroll)print*,'sedflux: N4toN3 = KNH4**2*ThtaN4**(Temps - 20)/s*(O20/2.)/(KMO_N4 + (O20/2.))'  &
!     &                  ,KNH4,ThtaN4,Temps,s,O20,KMO_N4
      N4toN3=0.0
      if((s.ne. 0.0).and.(O20.ne. 0.0)) N4toN3 = KNH4**2*ThtaN4**(Temps - 20)/s*(O20/2.)/(KMO_N4 + (O20/2.))
!*KM_NH4/(KM_NH4 + NH4(1))
...
!  Schicht 2
 ...
 b2 = -JN
      ...
      !if(kontroll)print*,'sedflux: Lin_Sys(a11,a12,a21,a22,b1,b2',a11,a12,a21,a22,b1,b2
 ...
 fd1_aus = fd1 
      ...
      !if(kontroll)print*,'sedflux: NSOD =fO_Nit*N4toN3*NH4(1)',fO_Nit,N4toN3,NH4(1)
...
!  Denit(1) = sqrt(KappN3*ThtaN3**(Temps-20.)*s)        ! ich
      Denit(1) = 0.0
      if((s*hconD1).ne. 0.0)Denit(1) = KappN3**2*ThtaN3**(Temps-20.)/s*hconD1           ! nach DiToro
...
!
  JC4N31 = JC1*hconD1*NO3(1)/(NO3(1)+KM_NO3)
      !if(kontroll)print*,'sedflux: JC4N32 = JDenit(2)*fdnC_N*fC_O2',JDenit(2),fdnC_N,fC_O2
...
!
      ...
      !if(kontroll)print*,'sedflux: Jcneu = Jc-JC4N32',Jc,JC4N32
 Jcneu = Jc-JC4N32
   if(Jcneu.lt.0.0)Jcneu = 0.0

      !if(kontroll)print*,'sedflux: CSODmx = sqrt(2.*KL12*CH4sat*Jcneu)',KL12,CH4sat,Jcneu
 CSODmx = sqrt(2.*KL12*CH4sat*Jcneu)
   if(Jcneu.lt.CSODmx)CSODmx = Jcneu
      hcon1 = 0.0
      hcon2 = 0.0
      if(SOD.ne. 0.0)then
...
!
   if(hcon1.gt.400.)then
      CSOD = CSODmx
         else
      sech = 2./(exp(hcon1)+exp(-hcon1))
      !if(kontroll)print*,'sedflux: CSOD = CSODmx*(1.-sech)',CSODmx,sech
      CSOD = CSODmx*(1.-sech)
         endif
!
      !if(kontroll)print*,'sedflux: CSOD1 = Jc1 - JC4N31',Jc1,JC4N31
      CSOD1 = Jc1 - JC4N31
!
      SODinit = SOD
...
!   Vaenderung des leichtabbaubaren Kohlenstoffsgehalts in der aeroben Schicht durch Sedimentation im Zeitschritt<tflie>
!
