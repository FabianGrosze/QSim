...
!...2D-Modellierung                                                     
     ...
     enddo         ! 2D Schleifenende 
      ...
      Psed = 0.0
      if(Tiefe(ior).gt. 0.0) Psed = hJPO4(mstr,ior)*tflie/Tiefe(ior) 
...
!....1D-PO4-Konzentration                                               
        ...
        if(nkzs(ior)>1)then
           ...
           if(sumH.gt. 0.0) gelPt = gelPt+sagP/sumH+sakP/sumH+sabP/sumH                                                         
        else
           gelPt = gelP(ior)+(agrP(1)+akiP(1)+ablP(1))+dop+Psed+gelPzo+gelPdr                                                           
        endif

        delp = gelPt-gelP(ior) 
        if(gelPt.lt. 0.0)then
           gelPt=0.0
           if( (gelP(ior)+abs(delp)).gt. 0.0) gelPt = ( gelP(ior)/ (gelP(ior)+abs(delp)) ) *gelP(ior)                                                        
        endif
