...
subroutine erosion(ss, ssalg, SSeros, dsedH, tausc, M_eros, n_eros, sedroh,         &
   ...
   do ior = 1,anze+1
      ...
      call erosion_kern(tflie,TIEFE(ior),RAU(ior),VMITT(ior),htau(mstr,ior)  &
                        ...
                        ,kontroll,ior,mstr)
	  ! 1  202 ! Elbe-Km 94.40000
      if((mstr==1) .and. (ior==202))print*,'erosion Elbe-Km 94.40TIEFE,RAU,VMITT,tau,tausc',  &
					TIEFE(ior),RAU(ior),VMITT(ior),htau(mstr,ior),tausc(mstr,ior)
      ! 2 316  ! 979-663   ! Elbe-Km 474,5
      !if((mstr==2) .and. (ior==316))print*,'erosion Elbe-Km 474,5 sseros,tau,tausc',SSeros(ior),htau(mstr,ior),tausc(mstr,ior)
      ! 2 512  ! 1175-663  ! Elbe-Km 585,05
      !if((mstr==2) .and. (ior==512))print*,'erosion Elbe-Km 585,05 sseros,tau,tausc',SSeros(ior),htau(mstr,ior),tausc(mstr,ior)
      if((mstr==2) .and. (ior==512))print*,'erosion Elbe-Km 585.05 TIEFE,RAU,VMITT,tau,tausc',  &
					TIEFE(ior),RAU(ior),VMITT(ior),htau(mstr,ior),tausc(mstr,ior)
